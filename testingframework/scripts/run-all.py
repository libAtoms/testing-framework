...
parser.add_argument(
    "--test_set", "-s", nargs="+", help="label(s) for tests directories", required=True
)
parser.add_argument(
    "--models", "-m", action="store", nargs="+", type=str, help="models to include"
)
parser.add_argument(
    "--tests", "-t", action="store", nargs="+", type=str, help="tests to include"
)
parser.add_argument(
    "--omit-tests", "-o", action="store", nargs="+", type=str, help="tests to omit"
)
parser.add_argument(
    "--force", "-f", action="store_true", help="force rerunning of tests"
)
parser.add_argument(
    "--bugs", "-b", action="store_true", help="use bugs to generate job scripts"
)
parser.add_argument(
    "--bugs_np", "-n", help="override number of processor when using bugs"
)
parser.add_argument(
    "--bugs_host",
    "-H",
    help="host to force bugs to use",
    default=re.sub(
        "[0-9]*$", "", os.environ.get("HOSTNAME", "None.FQDN").split(".")[0]
    ),
)
...
parser.add_argument(
    ...
    action="store",
    type=str,
    help="path to tests directory (above test_set)",
    default=os.path.join(my_path, "../tests"),
)
parser.add_argument(
    "--n_procs",
    "-N",
    action="store",
    type=int,
    help="number of processors to round to",
    default=16,
)
...
if args.models is not None:
    ...
    models = list(
        itertools.chain.from_iterable(
            [
                glob.glob(os.path.join(args.models_path, d, "model.py"))
...
for test_set in args.test_set:
    ...
    if args.tests is not None:
        tests_t = list(
            itertools.chain.from_iterable(
                [glob.glob(os.path.join(tests_path, d, "test.py")) for d in args.tests]
            )
        )
    else:
    ...
    if args.omit_tests is not None:
        for omit_test in list(
            itertools.chain.from_iterable(
                [glob.glob(os.path.join(tests_path, d)) for d in args.omit_tests]
            )
        ):
            if omit_test in tests_t:
                ...
                sys.stderr.write(
                    "WARNING: omitted test %s not in list of tests\n" % omit_test
                )
...
for model in models:
    ...
    args = parser.parse_args(
        sys.argv[1:] + model_default_run_opts + global_default_run_opts
    )
    ...
    for test, test_set in tests:
        ...
        if args.bugs_np is None:
            np = (
                int(
                    round(
                        int(test_cost * model_cost * args.n_procs) / float(args.n_procs)
                    )
                )
                * args.n_procs
            )
        ...
        if args.bugs:
            ...
            cmd_args = [
                clean_path(run_model_test),
                clean_path(os.path.join(args.models_path, model_name)),
                clean_path(test),
            ]
        else:
            cmd_args = [
        ...
        if args.bugs:
            ...
            cmd += [
                "-time=96h",
                f"-np={np}",
                f"-script={bugs_script}",
                f"-host={args.bugs_host}",
                f"-name={ident_string}",
                f"-fileroot={ident_string}",
                "-in_cwd",
                f"-output=job.{ident_string}",
            ]
