...
(args, models, chem_order_tests, default_analysis_settings) = analyze_start(
    "chemical_order_*"
)
...
for model_name in models:
    ...
    for chem_order_test_name in chem_order_tests:
        ...
        prop_filename = "{}-model-{}-test-{}-properties.json".format(
            args.test_set, model_name, chem_order_test_name
        )
        try:
    ...
    for chem_order_test_name in chem_order_tests:
        ...
        if chem_order_test_name not in data[model_name]:
            sys.stderr.write(
                "skipping struct {} in plotting model {}\n".format(
                    chem_order_test_name, model_name
                )
            )
            continue
        ...
        if model_name != ref_model_name:
            if "unrelaxed_energy_per_atom" in data[model_name][chem_order_test_name]:
                (line,) = plot(
                    data[ref_model_name][chem_order_test_name][
                        "unrelaxed_energy_per_atom"
                    ],
...
for chem_order_test_name in figure_nums:
    ...
    ylabel("E (eV/atom)")
    x_range = (
        min(data[ref_model_name][chem_order_test_name]["unrelaxed_energy_per_atom"]),
