...
for model_name in models:
    ...
    for test_name in tests:
        ...
        try:
            (cur_min_EV, cur_composition) = read_ref_bulk_model_struct(
                args.test_set,
                model_name,
            ...
            continue
        try:
            (stable_mu_extrema, full_mu_range) = mu_range(
                cur_min_EV,
                cur_composition,
                data[model_name][test_name]["bulk_struct_test"],
                mcc_compositions,
                mcc_energies[model_name],
            )
        except:
        ...
        for defect_label in data[model_name][test_name]["defects"]:
            ...
            if "dmu" in defect:
                ...
                if stable_mu_extrema is None or len(stable_mu_extrema) == 0:
                    print(
                        f"DEFECT {model_name}, {test_name}, {ind}, {Z}, (Ef0 = {Ef0:.4f} Ef = {Ef:.4f}) but no stable mu range exists"
                    )
                else:
...
defects_sorted_print = [
    n.replace("point_defect_", "").replace("_", " ") for n in defects_sorted
]
print("\\begin{tabular}{ l & " + " & ".join(["c"] * 2 * len(defects_sorted)) + " }")
print(
    "model & "
    + " & ".join(
        ["\\multicolumn{{2}}{{c}}{{{}}}".format(n) for n in defects_sorted_print]
    )
    + "\\\\"
)
...
for model_name in models:
    ...
    for d in defects_sorted:
        if model_name in defect_table_data[l]:
            print(
                " & "
                + "{}".format(defect_table_data[d][model_name][0])
                + " & "
                + "{}".format(defect_table_data[d][model_name][1]),
                end="",
            )
        else:
            print(" & & ", end="")
