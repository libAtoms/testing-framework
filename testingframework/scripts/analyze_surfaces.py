...
for model_name in models:
    ...
    for test_name in tests:
        ...
        if dmus is not None and any(
            [dmus[mu_Z] != 0.0 for mu_Z in dmus]
        ):  # multi component
            ...
            for mu_Z, n_dmu in dmus.items():
                if n_dmu != 0:
                    ...
                    l_dmu += " + ( {:.4f} * mu_{} = [ {:.4f} -- {:.4f} ] )".format(
                        n_dmu, mu_Z, mu_contrib_min, mu_contrib_max
                    )
                    mu_contrib_min_total += mu_contrib_min
                    mu_contrib_max_total += mu_contrib_max
            l_dmu += " = [ {:.4f} -- {:.4f} ]".format(
                data[model_name][test_name]["Ef"] + mu_contrib_min_total,
...
surfaces_sorted_print = [
    n.replace("surface_", "").replace("_", " ") for n in surfaces_sorted
]
