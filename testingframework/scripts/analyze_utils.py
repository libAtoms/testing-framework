...
def get_matching_from_test_sets(test_sets, filename, unique=False):
    ...
    if unique:
        if len(files) > 1:
            raise RuntimeError(
                f"unique but more than one file for {test_sets}, {filename} = {files}"
            )
        elif len(files) == 0:
            return None
        else:
...
def read_ref_bulk_model_struct(test_set, model_name, struct_name):
    ...
    with open(
        get_matching_from_test_sets(
            test_set,
            f"model-{model_name}-test-{struct_name}-properties.json",
            unique=True,
        ),
        "r",
    ) as f:
    ...
    at = ase.io.read(
        get_matching_from_test_sets(
            test_set, f"model-{model_name}-test-{struct_name}-relaxed.xyz", unique=True
        ),
        format="extxyz",
    )
...
def get_element_ref_structs(test_set, models, element_ref_struct):
    ...
    for symb in element_ref_struct:
        ...
        for model_name in models:
            try:
                (min_EV, composition) = read_ref_bulk_model_struct(
                    test_set, model_name, struct_name
                )
            except:
                continue
...
def get_multicomponent_constraints(
    test_set, models, multicomponent_constraints_structs
):
    composition_data = {}
    energy_data = {}
    if debug:
        print(
            "get_multicomponent_constraints",
            multicomponent_constraints_structs,
            isinstance(multicomponent_constraints_structs, str),
        )
    ...
    for model_name in models:
        ...
        if isinstance(multicomponent_constraints_structs, str):
            ...
            struct_name_list = get_matching_from_test_sets(
                test_set,
                f"model-{model_name}-test-{multicomponent_constraints_structs}-properties.json",
            )
            struct_name_list = [
                re.sub(f".*-model-{model_name}-test-", "", x).replace(
                    "-properties.json", ""
                )
                for x in struct_name_list
            ]
        else:
        ...
        for struct_name in struct_name_list:
            if debug:
                print(
                    "get_multicomponent_constraints struct_name",
                    test_set,
                    model_name,
                    struct_name,
                )
...
def analyze_start(default_tests=["*"]):
    ...
    parser.add_argument(
        ...
        nargs="+",
        action="store",
        help="label for tests directories",
        required=True,
    )
    parser.add_argument(
        "--models_path",
    ...
    parser.add_argument(
        ...
        type=float,
        action="store",
        help="offset of reference model energy/atom",
        default=0.0,
    )
    ...
    models = [
        ...
        for f in list(
            itertools.chain.from_iterable(
                [glob.glob(os.path.join(args.models_path, d)) for d in args.models]
            )
        )
    ]
    ...
    for test_set in args.test_set:
        ...
        tests.extend(
            [
                ...
                for f in list(
                    itertools.chain.from_iterable(
                        [glob.glob(os.path.join(tests_path, d)) for d in args.tests]
                    )
                )
            ]
        )
        ...
        for test_name in tests:
            ...
            try:
                prop_filename = get_matching_from_test_sets(
                    test_set,
                    f"model-{model_name}-test-{test_name}-properties.json",
                    unique=True,
                )
            except:
                print(
                    "Failed to get unique prop_filename for ",
                    test_set,
                    f"model-{model_name}-test-{test_name}-properties.json",
                )
                continue
