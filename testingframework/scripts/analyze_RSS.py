...
(args, models, tests, default_analysis_settings) = analyze_start(
    "gap_6_rss_like_open_ended_minima_rerelax_sd2"
)
...
A, B = (
    rss_data[ref_model_name]["gap_6_rss_like_open_ended_minima_rerelax_sd2"][
        "V_unrelaxed"
    ],
    rss_data[ref_model_name]["gap_6_rss_like_open_ended_minima_rerelax_sd2"][
        "E_unrelaxed"
...
for (i, model) in enumerate(models):
    if model != ref_model_name:
        C, D = (
            rss_data[model]["gap_6_rss_like_open_ended_minima_rerelax_sd2"][
                "V_relaxed"
            ],
            rss_data[model]["gap_6_rss_like_open_ended_minima_rerelax_sd2"][
                "E_relaxed"
        ...
        a0.add_collection(lc)
        a0.plot(
            C, D, "_", marker="o", color=struct_colors[i], markersize=2, label=model
        )
