...
def get_rdf(
    ...
    if (
        save
        and os.path.isfile("midpoints_{}.txt".format(uid))
        and os.path.isfile("array-2d_{}.txt".format(uid))
    ):
        if verbose:
            print(
                "Found data, loading from files: `midpoints_{uid:}.txt` & `array-2d_{uid:}.txt`".format(
                    uid=uid
                )
            )
    ...
    if type(s1) is int and type(s2) is int:
        if verbose:
            print(
                "Setting cutoff to species pair ({},{}), with range {}".format(
                    s1, s2, cutoff_size
                )
            )
    ...
    for j, atoms in enumerate(atoms_list):
        ...
        count_normd_2d[j, :] = (
            (count / (4.0 * np.pi / 3.0 * (bin_edges[1:] ** 3 - bin_edges[:-1] ** 3)))
            / rho
        ) / n_particles

        if verbose:
            sys.stdout.write(
                "Index: {:4.0f} | Number of pairs: {:>6} | {:>5.1f}%\r".format(
                    j, len(list_d), ((j + 1) / len(atoms_list)) * 100
                )
            )
            sys.stdout.flush()
    if verbose:
...
def block_average(
    ...
    array_2d, midpoints = get_rdf(
        atoms_list, cutoff_size, s1, s2, bins, save, comment, verbose
    )
