...
def do_surface(test_dir):
    ...
    bulk_model_test_root = model_test_root(model.__file__, bulk_test_name)

    print "get bulk from ",'../%s-relaxed.xyz' % bulk_model_test_root
    ...
    tol = 1.0e-3
    surf = relax_config(surf, relax_pos=True, relax_cell=False, tol=tol, traj_file=None, label="surface", from_base_model=True, save_config=True)
    ...
    # return (bulk_model_test_root, (surf.get_potential_energy() - bulk.info['energy']*n_fu)/(2.0*area) )
    return (bulk_model_test_root, (surf.get_potential_energy() - bulk.get_potential_energy()*n_fu)/(2.0*area) )
