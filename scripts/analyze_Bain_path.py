...
for test in tests:
    ...
    ax.plot([v[0] for v in ref_Bain], [v[1] for v in ref_Bain], label=ref_model_name, linestyle=ref_linestyles[0], color="black")
    ...
    for model in sorted(Bain_data.keys()):
        if bool(Bain_data[model]) is True and model != ref_model_name:
            i += 1
            ax.plot([v[0] for v in Bain_data[model][test]['Bain_path_E']], [v[1] for v in Bain_data[model][test]['Bain_path_E']],
                    label=model, color=struct_colors[i], linestyle=other_linestyles[0])
